...
using TrackFinder.ViewModels.Community;
...
public class CommunityController : Controller
{
    ...
    public async Task<IActionResult> Index(string filter = "all", Guid? communityId = null)
    {
        ...
        var model = new CommunityFeedViewModel
        {
            ActiveFilter = filter,
            SelectedCommunityId = selectedCommunityId,
            CurrentUserId = currentUser?.Id,
            CurrentStudentId = currentStudentId,
            NewPost = new PostFormViewModel
            {
    ...
    public async Task<IActionResult> CreatePost(PostFormViewModel input)
    {
        var currentUser = await GetCurrentUserAsync();
    ...
    [HttpPost]
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> EditPost(Guid id, PostFormViewModel input)
    {
        if (id != input.Id)
        {
            return BadRequest();
    ...
    [HttpPost]
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> AddComment(CommentFormViewModel input)
    {
        var currentUser = await GetCurrentUserAsync();
        if (currentUser is null)
        {
    ...
    private async Task<IReadOnlyList<CommunitySummaryViewModel>> BuildCommunitiesAsync(Guid? currentStudentId)
    {
        ...
        return communities.Select((community, index) => new CommunitySummaryViewModel
        {
            Id = community.Id,
            Name = community.Name,
    ...
    private static PostCardViewModel ToPostCard(Post post, Guid? currentUserId)
    {
        return new PostCardViewModel
        {
            ...
            Comments = post.Comments
                ...
                .Take(2)
                .Select(c => new CommentPreviewViewModel
                {
                    Id = c.Id,
                    Content = c.Content,
    ...
    private static IReadOnlyList<TopicSummaryViewModel> BuildTrendingTopics(IEnumerable<Post> posts)
    {
        var topics = posts
            .SelectMany(p => ExtractTags(p.Content))
            .GroupBy(tag => tag, StringComparer.OrdinalIgnoreCase)
            .Select(group => new TopicSummaryViewModel
            {
                Name = group.Key,
        ...
        return topics.Count > 0
            ? topics
            : [new TopicSummaryViewModel { Name = "DotNet", PostsCount = 0 }];
    }
